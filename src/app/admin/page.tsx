  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#9d7837]">Plataforma</p>
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-neutral-950">Barberías</h1>
          <p className="mt-1 text-sm text-neutral-500">{barbershops?.length ?? 0} registradas</p>
        </div>
        {pendingPayments ? (
          <Link
            href="/admin/pagos"
            className="rounded-full bg-amber-50 px-3.5 py-2 text-sm font-semibold text-amber-700 hover:bg-amber-100"
          >
      ...
      <div className={`divide-y divide-[#eeeae2] ${CARD}`}>
        {(barbershops ?? []).map((b) => (
          <Link
            key={b.id}
            href={`/admin/${b.id}`}
            className="flex items-center justify-between px-5 py-4 transition hover:bg-[#fffaf0]"
          >
            <div>
            ...
            <span
              className={`rounded-full px-2.5 py-1 text-xs font-semibold ${STATUS_COLORS[b.status] ?? "bg-neutral-100 text-neutral-600"}`}
            >
              {b.status}
            </span>
          </Link>
