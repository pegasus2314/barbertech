...
**Datos bancarios + QR en el pago de suscripción** (`src/lib/subscription/bank-details.ts`, `src/app/dashboard/[tenant]/configuracion/bank-transfer-details.tsx`): el formulario para registrar un pago de suscripción (`subscription-payment-form.tsx`) nunca mostró a dónde transferir — el dueño de la barbería tenía que preguntarlo por fuera. Se agregó una tarjeta con los datos de la cuenta bancaria (banco, tipo de cuenta, titular, cédula, número de cuenta, cuenta estándar/IBAN, SWIFT) más un código QR generado en el servidor (`qrcode`, sin dependencias con vulnerabilidades) que codifica esos mismos datos como texto plano — no es un QR de pago bancario real (República Dominicana no tiene un estándar interbancario tipo Pix/CoDi accesible aquí), solo evita que el dueño tenga que escribir el número de cuenta a mano al transferir. Los datos viven en variables de entorno (`SUBSCRIPTION_BANK_*`, sin prefijo `NEXT_PUBLIC_` aunque terminen visibles en la página — se leen server-side en un Server Component, nunca se exponen en el bundle del cliente) en vez de hardcodeados en el código, para no dejar un número de cuenta real committeado en el repositorio.
