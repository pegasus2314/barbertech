...
La app no enviaba ninguna cabecera de seguridad (`next.config.ts` estaba vacío). Se agregó un `headers()` en `next.config.ts` que aplica, solo en producción:

- **Content-Security-Policy**: `default-src 'self'` con excepciones puntuales — `img-src`/`connect-src` permiten el dominio del proyecto de Supabase (API REST + Realtime por `wss://`), `script-src`/`style-src` incluyen `'unsafe-inline'` (Next.js App Router y Tailwind lo necesitan; no se armó un esquema de nonces por ahora), `worker-src 'self'` para el service worker, y `object-src 'none'` + `frame-ancestors 'none'` para bloquear plugins y que el sitio no pueda embebirse en un iframe ajeno (clickjacking).
- **X-Content-Type-Options: nosniff**, **X-Frame-Options: DENY**, **Referrer-Policy: strict-origin-when-cross-origin**, **Permissions-Policy** (bloquea cámara/micrófono/geolocalización, que la app no usa), y **Strict-Transport-Security** (fuerza HTTPS, seguro ahora que `barbertech.site` ya tiene certificado real).

**Por qué solo en producción**: el CSP bloquea `eval()`, que las herramientas de depuración de React en modo desarrollo sí usan (React nunca lo usa en producción) — con el CSP activo en `next dev`, hasta el websocket de hot-reload se rompía. `headers()` revisa `NODE_ENV` y devuelve un arreglo vacío fuera de producción.

**Falso positivo durante las pruebas**: al verificar con `next start` en un puerto manual (3100, fuera del servidor de vista previa normal), el registro del service worker fallaba con "An unknown error occurred when fetching the script". Se sospechó del CSP, pero se descartó por completo probando: (1) que no se disparaba ningún evento `securitypolicyviolation`, y (2) que el fallo persistía incluso quitando *todas* las cabeceras personalizadas de `/sw.js`, dejando solo una de prueba. Como el mismo registro funciona sin problema en `next dev` (puerto 3000, el que sí maneja el panel de vista previa), se concluyó que es una particularidad de probar un puerto ad-hoc en este entorno, no un problema real de las cabeceras — confirmado además contra el sitio ya desplegado en `barbertech.site`.

## 15. Límite de intentos (rate limiting) en el flujo público de reservas

Crear cita, cancelar cita y consultar citas por teléfono (`/[slug]/reservar`, `/[slug]/mi-cita`) son públicas — cualquiera puede llamarlas sin cuenta, así que nada impedía que un script spameara la agenda de una barbería con citas falsas o abusara del buscador de citas.

**Mecanismo** (`src/lib/rate-limit.ts`): tabla `rate_limits` (migración [0011_rate_limits.sql](supabase/migrations/0011_rate_limits.sql)) con `key` + `action` + `created_at`. Antes de crear/cancelar/consultar una cita, `checkRateLimit` cuenta cuántos intentos hubo para esa combinación en los últimos 10 minutos; si ya llegó al límite, no inserta y devuelve `false` (el Server Action responde con "Demasiados intentos..." en vez de llamar la RPC real). Se revisa dos claves en paralelo por cada acción — `phone:<teléfono>` y `ip:<ip>` (la IP sale de `x-forwarded-for` vía `headers()` de Next) — porque limitar solo por teléfono se evade inventando números falsos, y limitar solo por IP castigaría a varios clientes reales reservando desde el wifi de la misma barbería. Límites: crear/cancelar cita, 5 por teléfono y 15 por IP cada 10 min; consultar citas (más inocuo), 10 por teléfono y 30 por IP. Vive enteramente en Postgres, sin Redis ni servicios de pago — se apoya en el cliente `service_role` que ya existía (`src/lib/supabase/admin.ts`), por eso la tabla tiene RLS activado sin ninguna política (deniega todo excepto al service role).

`checkRateLimit` **falla abierto**: si la consulta a la base de datos da error, deja pasar el intento en vez de bloquear reservas reales por un problema de infraestructura — mismo criterio que ya se usa en `sendPushToTenant`.

Verificado insertando filas de prueba directamente vía SQL y confirmando que el conteo cruza el límite en la fila exacta esperada; y de punta a punta reservando una cita real desde el navegador para confirmar que el flujo normal (dentro del límite) sigue funcionando sin fricción. No se pudo probar el bloqueo real end-to-end en local porque `SUPABASE_SERVICE_ROLE_KEY` está vacía en `.env.local` de este entorno (aunque sí existe en Vercel, confirmado en la sección de Web Push) — con la clave vacía, `checkRateLimit` falla abierto localmente, así que solo limita de verdad en producción.

No se agregó limpieza automática de filas viejas en `rate_limits` (la tabla crece indefinidamente); no es urgente porque el índice cubre bien la consulta por ventana reciente, pero es una mejora pendiente razonable (ej. una `pg_cron` que borre filas de más de un día).

## 16. Realtime en Citas + botones optimistas

**Realtime en Citas** (`src/app/dashboard/[tenant]/citas/realtime-refresh.tsx`): mismo patrón que la campanita de notificaciones (suscripción a `postgres_changes` en `appointments` filtrado por `tenant_id`, con `supabase.realtime.setAuth()` antes de suscribirse) pero en vez de insertar en una lista local, simplemente llama `router.refresh()` — mucho menos código que duplicar los `select(*, clients(...), services(...), barbers(...))` en el cliente, y el resultado visible es el mismo: la agenda se actualiza sola cuando entra una cita nueva o alguien le cambia el estado desde otro dispositivo, sin recargar la página. Deliberadamente no se agregó a Finanzas, Clientes ni Estadísticas — son pantallas que se consultan, no se vigilan en vivo, y cada suscripción extra sí tiene un costo real (ver bug abajo).

**Bug real encontrado en pruebas — la suscripción no fallaba, simplemente no recibía nada**: al probar cambiando el estado de una cita directamente por SQL (simulando otro dispositivo), la página abierta no se actualizaba sola, sin ningún error en consola ni en la suscripción. Diagnosticado consultando `pg_publication_tables` directamente: solo `notifications` estaba agregada a la publicación `supabase_realtime` — `appointments` nunca lo estuvo, así que Postgres no transmitía ningún cambio de esa tabla sin importar qué tan bien estuviera escrito el cliente. Corregido con `alter publication supabase_realtime add table public.appointments` (migración [0012_appointments_realtime.sql](supabase/migrations/0012_appointments_realtime.sql)). **Lección, igual que con el bug de RLS+Realtime de la campanita:** una suscripción "conectada" sin errores no prueba que vayan a llegar eventos — hay que probarlo escribiendo un cambio real y confirmando que aparece, cosa que efectivamente se hizo después de la corrección (una cita insertada por SQL apareció sola en la lista sin recargar).

**Botones optimistas** (`src/lib/use-optimistic-action.ts`): la mayoría de los botones de una sola acción en el dashboard (cambiar estado de una cita, activar/desactivar un servicio o barbero, anular un pago, publicar/despublicar la página) esperaban a que terminara el viaje al servidor y un `router.refresh()` completo antes de mostrar el nuevo estado — se sentía lento. El hook usa `useOptimistic` de React: el valor se muestra al instante al hacer clic, y si el Server Action falla, simplemente vuelve al valor real una vez termina la transición (sin lógica extra de "revertir"). Aplicado en `appointment-row.tsx`, `service-row.tsx`, `barber-row.tsx` (solo el toggle de activo — el multiselect de servicios ya era optimista desde antes), `payment-row.tsx` y `publish-toggle.tsx`. Deliberadamente no se tocaron los formularios con varios campos (crear/editar cliente, servicio, barbero, horarios, subir logo) — mostrar datos sin guardar como si ya estuvieran guardados es una categoría de bug distinta y más confusa que vale la pena evitar.

## 17. Personalización por barbería: color, portada, redes y nota de reserva

`barbershops.theme` y `barbershops.social_links` (columnas `jsonb`) existían desde la migración inicial pero ningún formulario del panel las tocaba — se agregó el formulario real, más una columna nueva `booking_note` (migración [0013_barbershop_booking_note.sql](supabase/migrations/0013_barbershop_booking_note.sql)).

**Qué se puede personalizar ahora** (`src/app/dashboard/[tenant]/configuracion/customization-form.tsx`, acción `updateCustomization`): color de acento propio (reemplaza el dorado de BarberTech en su página pública y su flujo de reservas — no en el panel del dueño, que sigue siendo BarberTech), oscuridad de la portada (clara/media/oscura), enlaces de Instagram/Facebook/TikTok (se muestran como íconos junto al botón de WhatsApp), y una nota corta que el cliente ve justo al confirmar su cita y que además se agrega al mensaje de WhatsApp de confirmación.
...
De las 4 plantillas de correo con marca propia (sección "Correo transaccional con marca propia" más arriba), "Reset Password" era la única con una plantilla lista pero sin ninguna forma de dispararla desde la app — no había link de "¿Olvidaste tu contraseña?" en ningún lado. ("Invite user" se conectó después, ver sección 19. "Magic Link" sigue sin usarse: no existe login sin contraseña como función real todavía, así que configurarla en Supabase no tendría efecto.)
...
**Bug real encontrado ya en producción, con el usuario probando en vivo**: el enlace de recuperación llevaba a la página principal en vez de a `/reset-password`. Causa: `redirectTo` se armaba con el helper `siteUrl()`, que lee `VERCEL_PROJECT_PRODUCTION_URL` — una variable que **solo existe en el servidor**. Como `/forgot-password` es un componente cliente, esa variable llegaba `undefined` al navegador y `siteUrl()` caía en su último fallback, `http://localhost:3000`. Supabase rechazaba ese redirect (correctamente, no está en la lista de URLs permitidas) y caía de vuelta al "Site URL" configurado — que además, por separado, todavía apuntaba al alias viejo de Vercel en vez de a `barbertech.site`, por eso el síntoma inicial parecía un problema de configuración de Supabase. Se corrigieron las dos cosas: el "Site URL" de Supabase se actualizó a `https://barbertech.site`, y el código se cambió para usar `window.location.origin` en vez de `siteUrl()` dentro de ese componente cliente. **Lección:** `siteUrl()` es seguro de usar en Server Components/Actions (`VERCEL_PROJECT_PRODUCTION_URL` sí existe ahí, confirmado antes vía `sitemap.xml`), pero nunca desde un componente `"use client"` — ahí siempre hay que usar `window.location.origin`.

## 19. Invitar a un barbero por correo (Invite user)

La plantilla "Invite user" existía pero no tenía ninguna función real detrás. Ahora sí: desde **Barberos**, además de "+ Nuevo barbero" (solo nombre, sin acceso al panel), está "+ Invitar por correo" (`invite-barber-form.tsx`) — el barbero recibe el correo de invitación, crea su contraseña, y entra directo al panel ya asignado a esa barbería con rol `barber`.

**Por qué no se usa `user_metadata` para autorizar** (`src/app/(auth)/invite/accept/actions.ts`): `supabase.auth.admin.inviteUserByEmail()` acepta un campo `data` que se guarda en `user_metadata` — pero **el propio usuario puede reescribir su `user_metadata` libremente** vía `auth.updateUser()`. Confiar en eso para decidir "a qué barbería se une" sería dejar que cualquiera se auto-asigne a cualquier tenant. En su lugar, la invitación real vive en una tabla nueva, `barber_invites` (migración [0014_barber_invites.sql](supabase/migrations/0014_barber_invites.sql)), escrita por el propio dueño (protegida por RLS con `has_role(tenant_id, array['owner','manager'])`, mismo patrón que ya usa la tabla `barbers`). Al aceptar, se busca la invitación `pending` que coincida con el **correo de la sesión ya autenticada** (no con nada que el cliente mande) — ese correo solo pudo autenticarse ahí porque de verdad recibió y abrió el enlace de Supabase.

**Flujo de aceptación** (`/invite/accept`, mismo patrón de sesión-vía-hash/código que `/reset-password`): al aceptar, crea en una sola pasada la `membership` (rol `barber`), la fila en `barbers` (con `membership_id` ya enlazado) y las relaciones `barber_services` de la invitación, y marca la invitación como `accepted`. Usa el cliente `service_role` (`createAdminClient()`) porque quien acepta todavía no es miembro de ese tenant — ninguna política RLS normal le dejaría insertar su propia membership.

**Bug de robustez encontrado en pruebas**: si `getPendingInvite()` o `acceptInvite()` fallan (ej. sin `SUPABASE_SERVICE_ROLE_KEY` en local), la página se quedaba congelada en "Verificando invitación..." para siempre — el `useEffect` no tenía `try/catch`, así que una excepción no manejada simplemente nunca resolvía el estado. Corregido envolviendo ambas llamadas en `try/catch` y cayendo a un estado de error visible.

**Qué no se pudo probar en local**: igual que el límite de reservas (sección 15), `inviteBarber` necesita `SUPABASE_SERVICE_ROLE_KEY`, vacía en `.env.local` local — se reprodujo el error exacto (`supabaseKey is required`) al probarlo ahí, confirmando que esta acción sí necesita la clave real (a diferencia del rate-limit, que falla abierto). Se verificó en local lo que no depende de esa clave: la pantalla de `/invite/accept` sin sesión ("Enlace inválido o vencido"). **Corrección**: en una versión anterior de este documento se afirmó que el envío real se había verificado en producción; no era cierto. Al probarlo después con una invitación real se encontró el bug de abajo.

**Bug real de producción — enlace de invitación rechazado**: `inviteUserByEmail` (server, service role) genera enlaces *implícitos* (`/invite/accept#access_token=…&refresh_token=…`), pero el cliente de navegador de `@supabase/ssr` es PKCE y `GoTrueClient._getSessionFromURL` lanza `Not a valid PKCE flow url` para ese tipo de URL, así que nunca se creaba la sesión y el invitado veía "Enlace inválido o vencido" (la invitación quedaba `pending` para siempre). Corregido: `/invite/accept` lee los tokens del hash y llama a `setSession()` explícitamente (y limpia la URL). Además, la página Barberos ahora lista las invitaciones pendientes con **Reenviar** (invite; si el correo ya tiene cuenta, cae a un correo de recuperación con el mismo `redirectTo`) y **Cancelar** (`status = revoked`).

## 20. Soporte (formulario en vez de WhatsApp)

La burbuja flotante ya no enlaza a un número personal. `SupportBubble` (`src/components/support-bubble.tsx`) abre un panel con correo + mensaje; la server action `sendSupportMessage` (`src/components/support-actions.ts`) valida, aplica rate limit por IP (5 por hora) y guarda en `support_messages` con el cliente admin (migración `0015`). La tabla tiene RLS solo para admins de plataforma (lectura y marcar resuelto) y se atiende en `/admin/soporte`. `OpenSupportLink` abre el mismo panel desde enlaces de texto (landing, términos, privacidad, políticas). El teléfono de la tienda pública de cada barbería (`barbershops.whatsapp`) es distinto y sigue igual.

## 21. Políticas de compra

`/politicas` explica compra, activación, garantía y devoluciones (servicio digital por suscripción, pago por transferencia/efectivo confirmado a mano, 30 días por pago, 3 días de gracia — `GRACE_DAYS` en `src/lib/subscription/access.ts`). Los números de reembolso (7 días en el primer pago, 5 días hábiles para devolver, 24 h hábiles para activar) son decisiones de negocio: si cambian, actualizar esa página. Está enlazada desde el pie de la landing y los Términos, y listada en `sitemap.ts` junto con Términos y Privacidad.
